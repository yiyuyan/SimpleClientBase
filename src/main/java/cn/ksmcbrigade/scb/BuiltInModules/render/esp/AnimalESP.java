...
public class AnimalESP extends Module {
    ...
    public void renderLevel(Minecraft mc, RenderLevelStageEvent event) {
        ...
        for (Animal player:Minecraft.getInstance().level.getEntitiesOfClass(Animal.class,new AABB(Minecraft.getInstance().player.getPosition(0),Minecraft.getInstance().player.getPosition(0)).inflate(Minecraft.getInstance().options.getEffectiveRenderDistance()*16))) {
