...
public class MobESP extends Module {
    ...
    public MobESP() throws IOException {
        super(MobESP.class.getSimpleName(),false,-1,new Config(new File(MobESP.class.getSimpleName()),get()),ModuleType.RENDER);
    }
    ...
    public void renderLevel(Minecraft mc, RenderLevelStageEvent event) {
        ...
        for (Mob player:Minecraft.getInstance().level.getEntitiesOfClass(Mob.class,new AABB(Minecraft.getInstance().player.getPosition(0),Minecraft.getInstance().player.getPosition(0)).inflate(Minecraft.getInstance().options.getEffectiveRenderDistance()*16))) {
